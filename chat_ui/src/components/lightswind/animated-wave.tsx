...
const AnimatedWave: React.FC<AnimatedWaveProps> = ({
  ...
  const createGroundPlain = useCallback(() => {
    ...
    const groundPlain = {
      ...
      simplex2D: null as ReturnType<typeof createNoise2D> | null,
      ...
      moveNoise: function (mouse: { x: number; y: number }) {
        if (!this.geometry || !this.simplex2D || !this.simplex3D || !this._originalPositions) return;
        ...
        for (let i = 0; i < positions.count; i++) {
          ...
          let zOffset = this.simplex2D(xoff_wave, yoff_wave) * this.scale; // `this.scale` (amplitude) controls height.
